...
import RootRecoveryGuard from "@/components/RootRecoveryGuard";
...
import { shouldUseSmartSearch } from "@/lib/natural-language-parser";
...
export default function HomePage() {
  ...
  // Real-time search trigger - always uses smart search
  useEffect(() => {
    const trimmed = searchQuery.trim();
    console.log('[Search Trigger] searchQuery:', trimmed);
    
    if (!trimmed) return;
    
    console.log('[Search Trigger] Scheduling runSearch in 500ms');
    const debounceTimer = setTimeout(() => {
      console.log('[Search Trigger] Calling runSearch with:', trimmed);
      runSearch(trimmed);
    }, 500); // 500ms debounce
  ...
  // Smart search function (natural language queries)
  const runSmartSearch = async (query: string, requestId: number) => {
    try {
      ...
      // Update regular search results
      setMemberResults(result.data.members || []);
      setFamilyResults(result.data.families || []);
      
      // Set result type based on what was returned
      if (result.data.members.length > 0 && result.data.families.length > 0) {
        setResultType("mixed");
      ...
      // Update Smart Reports results in real-time
      setReportResults(result.data.members || []);
      setReportCount(result.data.count || 0);
      
      // Auto-open Smart Reports modal if not already open
      ...
      if (result.correctedQuery) {
        console.log('Query corrected:', result.originalQuery, '->', result.correctedQuery);
        toast({
          kind: "info",
      ...
      if (requestId !== searchRequestSeq.current) return;
      setSearchError(e.message || "Smart search failed");
    } finally {
      if (requestId !== searchRequestSeq.current) return;
  ...
  // Search function - simplified to always use smart search
  const runSearch = async (query: string) => {
    ...
    console.log('[runSearch] Called with:', trimmed);
    ...
    try {
      // Always use smart search for all queries
      await runSmartSearch(trimmed, requestId);
    } catch (e: any) {
      if (requestId !== searchRequestSeq.current) return;
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && searchQuery.trim()) {
                  console.log('[Search Input] Enter pressed with:', searchQuery.trim());
              ...
              <div className="bg-white rounded-t-3xl shadow-2xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-300 print:hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-sm font-black text-neutral-400 uppercase tracking-widest">Smart Search</h4>
                    <div className="relative">
                      <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-emerald-400" />
                      <input
                        type="text"
                        placeholder="Type 'Widows', 'Moulavi', 'under 30', etc."
                        value={searchQuery}
                        onChange={(e) => {
                          console.log('[Modal Search Input] onChange:', e.target.value);
                          setSearchQuery(e.target.value);
                        }}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' && searchQuery.trim()) {
                            console.log('[Modal Search Input] Enter pressed with:', searchQuery.trim());
                            runSearch(searchQuery.trim());
                          }
                        }}
                        className="w-full pl-12 pr-4 py-3 bg-white border-2 border-emerald-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none"
                      />
                    </div>
                  </div>
