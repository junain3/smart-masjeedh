...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    console.log('DEBUG: Session found for:', session.user.email)
    ...
    try {
      ...
      if (roleError) {
        console.log('DEBUG: No user_roles found with auth_user_id, checking user_id column')
      ...
      console.error('DEBUG: Middleware database error:', error)
      // If database fails, allow access to prevent blocking
      return NextResponse.next()
    }

  } catch (error) {
    console.error('DEBUG: Middleware error:', error)
    // If middleware fails, allow access to prevent blocking
    return NextResponse.next()
  }
}
