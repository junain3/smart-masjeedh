...
-- Complete system for admin, co-admin, and staff permissions

-- Step 1: Update existing user_roles with proper permissions
UPDATE user_roles SET 
  permissions = CASE 
    WHEN role = 'super_admin' THEN '{"accounts": true, "events": true, "members": true, "subscriptions_collect": true, "subscriptions_approve": true, "staff_management": true, "reports": true, "settings": true}'
    WHEN role = 'co_admin' THEN '{"accounts": true, "events": true, "members": true, "subscriptions_collect": true, "subscriptions_approve": true, "reports": true}'
    WHEN role = 'staff' THEN '{"subscriptions_collect": true}'
    ELSE '{}'
  END,
  commission_percent = CASE 
    WHEN role = 'staff' THEN 10.0
    ELSE NULL
  END
...
SELECT 
  ur.masjid_id,
  ur.user_id,
  COALESCE(ur.commission_percent, 10.0)
FROM user_roles ur
WHERE ur.role = 'staff'
AND NOT EXISTS (
