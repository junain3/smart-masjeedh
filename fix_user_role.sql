-- Fix User Role - Run this in Supabase SQL Editor

-- First, create a masjid if not exists
INSERT INTO masjids (id, name, owner_id, created_at)
SELECT 
    gen_random_uuid(),
    'Test Masjid',
    u.id,
    NOW()
FROM auth.users u 
WHERE u.email = 'mohammedjunain@gmail.com'
AND NOT EXISTS (SELECT 1 FROM masjids WHERE owner_id = u.id)
RETURNING id;

-- Then, create user role
INSERT INTO user_roles (masjid_id, user_id, role, permissions, created_at)
SELECT 
    m.id,
    u.id,
    'super_admin',
    '{"accounts": true, "events": true, "members": true, "subscriptions_collect": true, "subscriptions_approve": true}',
    NOW()
FROM auth.users u
CROSS JOIN masjids m
WHERE u.email = 'mohammedjunain@gmail.com'
AND m.owner_id = u.id
AND NOT EXISTS (SELECT 1 FROM user_roles WHERE user_id = u.id);

-- Verify the fix
SELECT 
    u.email,
    u.email_confirmed_at,
    m.name as masjid_name,
    ur.role,
    ur.permissions
FROM auth.users u
JOIN masjids m ON m.owner_id = u.id
JOIN user_roles ur ON ur.user_id = u.id
WHERE u.email = 'mohammedjunain@gmail.com';
